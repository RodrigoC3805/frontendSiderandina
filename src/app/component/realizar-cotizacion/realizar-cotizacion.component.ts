  ...
  Swal.fire({
    ...
    didOpen: () => {
      ...
      swalEl?.querySelector('#btn-aceptar')?.addEventListener('click', () => {
        ...
        const detallesActualizar = this.cotizacionArray[realIndex].detalles.map((detalle, idx) => {
        ...
        this.cotizacionService.actualizarPreciosYEstado(
      ...
      swalEl?.querySelector('#btn-rechazar')?.addEventListener('click', () => {
        Swal.close();
        // Aquí puedes manejar la acción de rechazar si lo necesitas
      });
    }
  });
}
